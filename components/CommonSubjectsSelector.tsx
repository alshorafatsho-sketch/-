...
interface CommonSubjectsSelectorProps {
  onSubjectSelect: (subject: Subject) => void;
  selectedSubjectDisplayName: string | null;
}

const CommonSubjectsSelector: React.FC<CommonSubjectsSelectorProps> = ({ onSubjectSelect, selectedSubjectDisplayName }) => {
  const [isOpen, setIsOpen] = useState(false);
  
  const isSelected = commonSubjects.some(s => s.name === selectedSubjectDisplayName);
  const buttonText = isSelected ? selectedSubjectDisplayName : 'المواد المشتركة';
    ...
    <div className="relative inline-block text-left no-print">
      <div>
        <button
          ...
          id="common-subjects-menu"
          aria-haspopup="true"
          aria-expanded="true"
          onClick={() => setIsOpen(!isOpen)}
        >
          <BookOpenIcon className="mr-2 h-5 w-5 rtl:mr-0 rtl:ml-2" />
          {buttonText}
      ...
      {isOpen && (
        <div 
          ...
          aria-labelledby="common-subjects-menu"
          dir='rtl'
        >
...
export default CommonSubjectsSelector;